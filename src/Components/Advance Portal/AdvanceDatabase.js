...
const AdvanceDatabase = ({ username, userRoles = [], paymentModeOptions = [], refreshSignal, isActive = true }) => {
  ...
  const handleDelete = async (idToDelete) => {
    ...
    try {
      ...
      try {
        const loansRes = await fetch('https://backendaab.in/aabuildersDash/api/loans/all', {
          method: 'GET',
          credentials: 'include',
          headers: { 'Content-Type': 'application/json' },
        });
          ...
          if (linkedLoans.length) {
            ...
            const clearLoanRecord = async (loanRec) => {
              ...
              const res = await fetch(
                `https://backendaab.in/aabuildersDash/api/loans/${loanId}?editedBy=${encodeURIComponent(username)}`,
                {
                  method: 'PUT',
                  headers: { 'Content-Type': 'application/json' },
