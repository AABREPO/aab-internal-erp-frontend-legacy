...
const ADVANCE_PORTAL_FORM_FIELDS = {
  ...
  fromProjectName: 'From Project Name',
  toProjectName: 'To Project Name',
...
const AdvancePortal = ({
  ...
  const handlePaymentSubmit = async (skipDuplicateCheck = false) => {
    ...
    try {
      ...
      const advanceSaveUrl = withBranchUrl('https://backendaab.in/aabuildersDash/api/advance_portal/save');
    ...
    try {
      if (editFormData.type === "Transfer") {
        ...
        if (sameEntryRows.length === 2) {
          ...
          await Promise.all([
            fetch(`https://backendaab.in/aabuildersDash/api/advance_portal/edit/${editedRecord.advancePortalId}?editedBy=${encodeURIComponent(enteredBy)}`, {
              method: 'PUT',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify(updatedEdited)
            }),
            fetch(`https://backendaab.in/aabuildersDash/api/advance_portal/edit/${otherRecord.advancePortalId}?editedBy=${encodeURIComponent(enteredBy)}`, {
              method: 'PUT',
              headers: { 'Content-Type': 'application/json' },
        ...
        const res = await fetch(`https://backendaab.in/aabuildersDash/api/advance_portal/edit/${editingId}?editedBy=${encodeURIComponent(enteredBy)}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
                ...
                <div className='grid grid-cols-2 gap-3 text-left'>
                  ...
                  <div className='text-left'>
                    <div className='flex justify-between mb-[8px]'>
                      <label className="text-md font-semibold block">{ADVANCE_PORTAL_FORM_FIELDS.contractorVendor}<span className="text-[#E4572E]">*</span></label>
                  ...
                  <div className='text-left'>
                    ...
                    <Select
                      ...
                      placeholder={selectedType === 'Transfer' ? ADVANCE_PORTAL_FORM_FIELDS.fromProjectName : ADVANCE_PORTAL_FORM_FIELDS.projectName}
                      isSearchable={true}
                      value={selectedSite}
