...
const TelecomDatabase = ({ username, userRoles = [] }) => {
    ...
    useEffect(() => {
        const fetchTelecomData = async () => {
            try {
                const response = await axios.get(TELECOM_EXPENSES_ENDPOINT);
                setTelecomData(response.data || []);
                setFilteredData(response.data || []);
            } catch (err) {
                console.error('Error fetching telecom data:', err);
                setError('Failed to fetch telecom data');
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleExportExcel = () => {
        ...
        const worksheetData = filteredData.map((item, index) => ({
            ...
            'Service Number': item.utilityTypeNumber || '-',
        ...
        XLSX.writeFile(workbook, 'TelecomExpenses.xlsx');
    };
                        ...
                        <table className="w-full border-collapse">
                            <thead>
                                <tr className="bg-[#FAF6ED]">
                                    ...
                                    <td className="px-4 py-2 text-left font-semibold">Service Number</td>
                            ...
                            <tbody>
                                {loading ? (
                                    ...
                                    <tr>
                                        <td colSpan="13" className="text-center py-4">No telecom expenses found</td>
                                    </tr>
                                ) : (
