...
const WaterTab = ({ username, userRoles = [] }) => {
    ...
    useEffect(() => {
        const fetchWaterTaxPayments = async () => {
            try {
                const response = await axios.get('https://backendaab.in/aabuilderDash/expenses_form/utility/watertax');
    ...
    useEffect(() => {
        ...
        if (filters.service) {
            filtered = filtered.filter(project =>
                project.propertyDetails.some(property =>
                    property.waterTaxNo && property.waterTaxNo.toLowerCase().includes(filters.service.toLowerCase())
                )
            );
        }
