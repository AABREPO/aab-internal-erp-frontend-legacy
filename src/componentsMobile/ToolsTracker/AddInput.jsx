...
const AddInput = ({ user }) => {
  ...
  const sheetItemIdOptions = React.useMemo(() => {
    ...
    if (!effectiveItemName) return [];

    const found = toolsItemNameListData.find(
      (x) => (x?.item_name ?? x?.itemName) === effectiveItemName
    );
    ...
    return toolsItemIdFullData
      .filter((item) => {
        ...
        if (!linkedItemNameId || String(linkedItemNameId) !== String(itemNameId)) {
          return false;
        }
        const dbId = item?.id;
      ...
      .filter(Boolean);
  }, [addSheetForm.itemName, selectedItemName, toolsItemNameListData, toolsItemIdFullData, shouldShowItemIdInSheet]);

  // Suggest next Item ID based on existing ones (e.g. "DH 02" -> "DH 03")
  const getSuggestedNextItemId = React.useCallback((itemIdStrings) => {
    if (!Array.isArray(itemIdStrings) || itemIdStrings.length === 0) return '';
    let maxNum = -1;
    let bestPrefix = '';
    let padLength = 1;
    for (const s of itemIdStrings) {
    ...
    return bestPrefix ? `${bestPrefix} ${nextNumStr}`.trim() : String(nextNum);
  }, []);

  const suggestedNextItemIdForMain = React.useMemo(() => {
    ...
    const itemNameId = found?.id;
    if (!itemNameId) return '';
    const itemIds = toolsItemIdFullData
      .filter((item) => {
        const linkedItemNameId = item?.item_name_id ?? item?.itemNameId;
      ...
      .map((item) => (item?.item_id ?? item?.itemId ?? '').trim())
      .filter(Boolean);
    return getSuggestedNextItemId(itemIds);
  }, [selectedItemName, toolsItemNameListData, toolsItemIdFullData, getSuggestedNextItemId]);

  const suggestedNextItemIdForSheet = React.useMemo(() => {
    const effectiveItemName = addSheetForm.itemName || selectedItemName;
    if (!effectiveItemName) return '';
    const found = toolsItemNameListData.find(
      (x) => (x?.item_name ?? x?.itemName) === effectiveItemName
    ...
    const itemIds = toolsItemIdFullData
      ...
      .filter(Boolean);
    return getSuggestedNextItemId(itemIds);
  }, [addSheetForm.itemName, selectedItemName, toolsItemNameListData, toolsItemIdFullData, getSuggestedNextItemId]);
        ...
        <div className="flex gap-3">
          ...
          <div className="flex-1">
            ...
            <SearchableDropdown
              ...
              fieldName="Item ID"
              showAllOptions={true}
              suggestedNewValue={suggestedNextItemIdForMain}
              addNewLabel={suggestedNextItemIdForMain ? `${suggestedNextItemIdForMain}` : null}
            />
          </div>
        </div>
      </div>
